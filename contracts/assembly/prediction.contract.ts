...
export class PredictionMarketContract extends Contract {
  ...
  private handleLmsrBuy(from: Name, quantity: Asset, memo: string): void {
    ...
    if (outcome_is_yes) {
      ...
      market!.volume_yes += total_charge; // Track per-outcome volume
    } else {
      market!.q_no += delta_q;
      market!.volume_no += total_charge; // Track per-outcome volume
    }
  ...
  claim(market_id: u64, user: Name): void {
    ...
    if (market!.version >= 2) {
      ...
      // Total winning shares for this outcome (q_yes or q_no - current outstanding shares)
      // Note: q_yes/q_no are decremented when users sell shares back to the AMM
      const total_winning_shares: i64 = (winning_outcome_id == 0) ? market!.q_yes : market!.q_no;
      
      // Pool available to winners: total collateral in minus payouts already made (sells) minus fees
      // This ensures the pool correctly accounts for any LMSR sell payouts that have already been made
      let pool: i64 = market!.total_collateral_in - market!.total_collateral_out - market!.collected_fees;
      if (pool < 0) pool = 0;
